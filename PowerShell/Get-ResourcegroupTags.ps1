...
<#
.SYNOPSIS
This script get all tags on resourcegroup.
...
.EXAMPLE
  PS C:\> Get-ResourceGroupTags.ps1
  Launch the script
#>
...
(
  ...
  $Reportfile = 'export-ResourceGroupTags.json',
...
Try {
  ...
  #region Get resourcegroup tags
  New-Item $($TimeStamp + '_' + $Reportfile) -ItemType "file" -Force
  $result = Get-AzureRmResourceGroup | Select-Object ResourceGroupName,Tags
  $result | Add-Member -NotePropertyName TimeStamp -NotePropertyValue $TimeStamp
  foreach ($obj in $result) {
    ($obj | ConvertTo-Json -Compress) -replace ('^\[','') -replace ('\]$','') | out-File  -Append $($TimeStamp + '_' + $Reportfile) 
  }
  #endregion Get resourcegroup tags


  #region push result to storage account
  Select-AzureRmSubscription GEM-IS-PRD
  $Keys = Get-AzureRmStorageAccountKey -ResourceGroupName $rg -Name $SA
  $StorageContext = New-AzureStorageContext -StorageAccountName $SA -StorageAccountKey $Keys[$Keyindex].Value
...
Catch {
  ...
  Write-Error "Get-ResourceGroupTags : Error on line $ErrorLine. The error message was: $ErrorMessage"
}
#endregion ---body---