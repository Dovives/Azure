...
function InstallADFSFarm {
    ...
    param (
        ...
        [Parameter(Mandatory = $true)]
        [string] $DecryptionCertificateThumbprint
    )
    ...
    Install-AdfsFarm `
        ...
        -DecryptionCertificateThumbprint $DecryptionCertificateThumbprint;    
...
class cADFSFarm {
    ...
    [DscProperty(Mandatory)]
    [string] $DecryptionCertificateName;
    ...
    [void] Set() {
        ...
        if ($this.Ensure -eq [Ensure]::Present) {
            ...
            if (!$AdfsProperties) {
                ...
                $siteCert = Get-ChildItem -Path "cert:\LocalMachine\My\" -DnsName $this.CertificateName
                $signingCert = Get-ChildItem -Path "cert:\LocalMachine\My\" -DnsName $this.SigningCertificateName
                $decryptionCert = Get-ChildItem -Path "cert:\LocalMachine\My\" -DnsName $this.DecryptionCertificateName
                
                $AdfsFarm = @{
                    ServiceCredential = $this.ServiceCredential;
                    InstallCredential = $this.InstallCredential;
                    CertificateThumbprint = $siteCert.Thumbprint;
